...
void SentiMom::OnBar(const BarEventMsg& msg)
{
    ...
    if (!m_mrollingWindow.full())    return;

    if(this_return > m_mrollingWindow.Mean() + 2*m_MomThreshold * m_mrollingWindow.StdDev()){
    ...
    //}
    else if (this_return < m_mrollingWindow.Mean() - 2*m_MomThreshold * m_mrollingWindow.StdDev())
	{
        //if(this_sma.s()>m_srollingWindow.Mean() - 2 * m_SentiThreshold * m_srollingWindow.StdDev()){
