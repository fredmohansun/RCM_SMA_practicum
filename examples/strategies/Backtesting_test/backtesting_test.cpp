...
void SimpleMomentum::RegisterForStrategyEvents(StrategyEventRegister* eventRegister, DateType currDate)
{
    for (SymbolSetConstIter it = symbols_begin(); it != symbols_end(); ++it) {
        eventRegister->RegisterForBars(*it, BAR_TYPE_TIME, 900);
    }
    TimeType now = TimeType(currDate);
    eventRegister->RegisterForRecurringScheduledEvents("check_trades", ConvertUSEasternToUTC(now), NULL_TIME_TYPE, boost::posix_time::minutes(15));
}
...
void SimpleMomentum::OnScheduledEvent(const ScheduledEventMsg& msg)
{
    if(msg.scheduled_event_name() == "check_trades"){
	    ...
	    double size = ins->last_trade().size();
            ostringstream str;
            str <<msg.fire_time()<< ":event fired! " << ins->symbol()<<"'s last trade at "<<price<<" with size: "<<size;
...
void SimpleMomentum::OnBar(const BarEventMsg& msg)
{
	...
	//logger().LogToClient(LOGLEVEL_DEBUG,"OnBar fired!");
        logger().LogToClient(LOGLEVEL_DEBUG, str.str().c_str());
    }
...
void SimpleMomentum::SendOrder(const Instrument* instrument, int trade_size)
{
    ...
    if(instrument->top_quote().ask()<.01 || instrument->top_quote().bid()<.01 || !instrument->top_quote().ask_side().IsValid() || !instrument->top_quote().ask_side().IsValid()) {
        std::stringstream ss;
        ss << "Sending buy order for " << instrument->symbol() << " at price " << instrument->last_trade().price() << " and quantity " << trade_size <<" with missing quote data";
        //logger().LogToClient(LOGLEVEL_DEBUG, ss.str());
        return;
    }
    */
    double price = trade_size > 0 ? instrument->last_trade().price() + m_aggressiveness : instrument->last_trade().price() - m_aggressiveness;
    std::cout<<"before, price = "<<price<<" size = "<<trade_size<<'\n'; 
...
void SimpleMomentum::Reprice(Order* order)
{
    ...
    params.price = (order->order_side() == ORDER_SIDE_BUY) ? order->instrument()->last_trade().price() + m_aggressiveness : order->instrument()->last_trade().price() - m_aggressiveness;
